...
************************************************
************************************************
***Indice Gini de ingresos y Curva de lorenz ***
...
//Nacional 
*(matrix cortada. Empieza en 2° fila, 1° columna)
mat gini=gini' 
matrix C=[gini[2..., 1...]]  	
...
graph twoway (line l0 p0, sort yaxis(1 2)) /// 
          ...
          title("Comparación de la Curva de Lorenz,según ámbito,2020") ///    
...
graph twoway  (line l1 p1, sort yaxis(1 2))  ///    
          ...
          (function y = x, range(0 100) yaxis(1 2)) ///    
          , aspect(1) xtitle("% acumulado población, p") ///    
          title("Comparación de la Curva de Lorenz, según sexo,2020") ///    
          ytitle("% acumulado ingresos") ///    
...
*Por grupo de edad 2020
drop p1 l1 p2 l2 
glcurve ipcm [aw=facpob] if g_edad==4 , pvar(p1) glvar(l1) lorenz nograph
glcurve ipcm [aw=facpob] if g_edad==5, pvar(p2) glvar(l2) lorenz nograph
glcurve ipcm [aw=facpob] if g_edad==6 , pvar(p3) glvar(l3) lorenz nograph
glcurve ipcm [aw=facpob] if g_edad==7, pvar(p4) glvar(l4) lorenz nograph
...
graph twoway  (line l1 p1, sort yaxis(1 2))  ///    
            (line l2 p2, sort yaxis(1 2))   /// 
			(line l3 p3, sort yaxis(1 2))   /// 
			(line l4 p4, sort yaxis(1 2))   /// 
          (function y = x, range(0 100) yaxis(1 2)) ///    
          , aspect(1) xtitle("% acumulado población, p") ///    
          title("Comparación de la Curva de Lorenz, según grupo de edad,2020") ///    
          ytitle("% acumulado ingresos") ///    
          legend(label(1 "De 18 a 29") label(2 "De 30 a 45") label(3 "De 46 a 60") label(4 "Más de 60") label(5 "Equidistribución") ) 

graph export "${graficos}//graph03.png", width(1000) replace
